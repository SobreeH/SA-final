...
// รับ livestock_id จาก GET
$livestock_id = intval($_GET['id'] ?? 0);
if ($livestock_id <= 0) {
    err("ไม่พบ ID ปศุสัตว์");
}

// ดึงข้อมูลปัจจุบัน
$sql = "SELECT * FROM Livestock WHERE livestock_id = ?";
$stmt = $conn->prepare($sql);
$stmt->bind_param("i", $livestock_id);
$stmt->execute();
$result = $stmt->get_result();
if ($result->num_rows === 0) err("ไม่พบข้อมูลปศุสัตว์นี้");
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // UPDATE
    if ($weight === null) {
        $sql = "UPDATE Livestock SET tag_number=?, type=?, breed=NULLIF(?, ''), weight=NULL, status=?, image=? WHERE livestock_id=?";
        $stmt = $conn->prepare($sql);
        $stmt->bind_param("ssssi", $tag_number, $type, $breed, $status, $imagePath, $livestock_id);
    } else {
        $sql = "UPDATE Livestock SET tag_number=?, type=?, breed=NULLIF(?, ''), weight=?, status=?, image=? WHERE livestock_id=?";
        $stmt = $conn->prepare($sql);
        $stmt->bind_param("sssissi", $tag_number, $type, $breed, $weight, $status, $imagePath, $livestock_id);
    }
    ...
    if ($stmt->execute()) {
        ...
        echo "<div class='alert alert-danger'>❌ แก้ไขไม่สำเร็จ: " . htmlspecialchars($stmt->error) . "</div>";
    }
    $stmt->close();
}
?>
